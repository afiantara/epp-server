...
public class ProdukSet {
	...
	public sf.inventory.ProdukSet[] getProdukSet(String cmpType) throws DAException
	{
		...
		try
		{
			String strSQL="SELECT a.*,b.nbarang,b.satuan FROM mprodukset a," + (cmpType.equals("EPP")?"mproduk" :"wproduk")   + " b where a.kbarang1=b.kbarang  and a.recstatus<>?";
			...
			for(int i=0; i < count;i++)
			{
				...
				item.setSatuan1(v.get(7).toString());
				items[i] = item;
			}
		}
		catch(Exception ex)
	...
	public sf.inventory.ProdukSet[] getProdukSet(String kbarang,String cmpType) throws DAException
	{
		...
		try
		{
			...
			log.info("getProdukSet->" + strSQL+ ",[0]" + kbarang);
			...
			for(int i=0; i < count;i++)
			{
				...
				item.setTglupdate(Long.parseLong(v.get(4).toString()));
				item.setUserupdate(v.get(5).toString());
				item.setNbarang(v.get(6).toString());
	...
	public boolean insertProdukSet(sf.inventory.ProdukSet produk) throws DAException
	{
		...
		log.info("insertProdukSet->" + strSQL+ ",[0]" + produk.getKbarang() + "[1]" + produk.getKbarang1() + "[2]" + produk.getRecstatus());
		try {
			PreparedStatement p=db.getStatement(strSQL);
	...
	public boolean updateProdukSet(sf.inventory.ProdukSet produk) throws DAException
	{
		...
		String strSQL="UPDATE mprodukset SET kqty=?,recstatus=?,tglupdate=?,userupdate=? where kbarang=? and kbarang1=?";
		log.info("updateProdukSet->" + strSQL+ ",[0]" + produk.getKbarang() + "[1]" + produk.getKbarang1() + "[2]" + produk.getRecstatus() + "[3]" + produk.getKqty());
		try {
			PreparedStatement p=db.getStatement(strSQL);
			p.setDouble(1, produk.getKqty());
			p.setString(2, produk.getRecstatus());
			p.setLong(3, produk.getTglupdate());
			p.setString(4, produk.getUserupdate());
			p.setString(5, produk.getKbarang());
